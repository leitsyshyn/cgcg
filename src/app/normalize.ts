...
import { sortedOrder } from '../geometry/delaunayDivideConquer';

export function normalizePoints(points: readonly AppPoint[]): AppResult<readonly AppPoint[]> {
  const errors: string[] = [];
  const seen = new Map<string, AppPoint>();

  points.forEach((item, index) => {
    if (!Number.isFinite(item.point.x) || !Number.isFinite(item.point.y)) {
      errors.push(`${item.name || `point ${index + 1}`} has non-finite coordinates.`);
    }

    const key = `${item.point.x},${item.point.y}`;
    const duplicate = seen.get(key);
    if (duplicate) {
      errors.push(`${item.name} duplicates ${duplicate.name} at (${item.point.x}, ${item.point.y}).`);
    } else {
      seen.set(key, item);
    }
  });
  ...
  if (errors.length > 0) {
    return { ok: false, error: { message: 'Unsupported input.', details: errors } satisfies AppError };
  }

  const order = sortedOrder(geometryPoints);
  const sortedIndexByOriginal = new Map(order.map((originalIndex, sortedIndex) => [originalIndex, sortedIndex]));
